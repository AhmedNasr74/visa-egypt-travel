...
@section('content')
    <form action="{{ route('dashboard.customized-categories.update' , $customizedCategory) }}" method="POST" class="page-body">
        @csrf
        @method('PUT')

        <!-- Container-fluid starts-->
        <x-dashboard.partials.breadcrumb title="Edit CustomizedCategory" :hideFirst="true">
        ...
        <div class="container-fluid">
            <div class="row">
                        ...
                        <x-dashboard.form.language-multi-tab-card tab-id="customized-categories">
                            @foreach(config('translatable.supported_locales') as $localKey => $local)
                                <div @class(['tab-pane fade', 'active show' => $localKey == config('app.locale')])
                                     ...
                                     <x-dashboard.form.input-text error-key="{{$localKey}}.title" name="{{$localKey}}[title]" :value="$customizedCategory->translateOrNew($localKey)->title" id="{{$localKey}}-title" label-title="Title"/>
                ...
                <div class="card-body needs-validation">
                    ...
                    :images="$customizedCategory->featured_image"
                    name="featured_image"/>

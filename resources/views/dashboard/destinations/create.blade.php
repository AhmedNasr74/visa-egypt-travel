    ...
    <form action="{{ route('dashboard.destinations.store') }}" method="POST" class="page-body">
        ...
        <div class="container-fluid">
            <div class="row">
                            ...
                            @foreach (config('translatable.supported_locales') as $localKey => $local)
                                <div @class([
                                    ...
                                    <x-dashboard.form.input-text error-key="{{ $localKey }}.slug"
                                        name="{{ $localKey }}[slug]" id="{{ $localKey }}-slug" label-title="Slug" />

                                    <x-dashboard.form.input-editor error-key="{{ $localKey }}.description"
                                        name="{{ $localKey }}[description]" id="{{ $localKey }}-description"
                ...
                <div class="card tab2-card">
                    <div class="card-body">
                        <x-dashboard.form.multi-tab-card :tabs="['featured', 'media']" tab-id="featured-media">
                            <div class="tab-pane fade active show" id="{{ 'featured-media-0' }}" role="tabpanel"
                                aria-labelledby="{{ 'featured-media-0' }}-tab">
                                <x-dashboard.form.input-checkbox resource-name="Destination" :value="true"
                                    error-key="enabled" name="enabled" id="enabled" label-title="Enabled" />

                                <x-dashboard.form.input-checkbox resource-name="Destination" resource-desc="Set Featured"
                                    error-key="featured" name="featured" id="featured" label-title="Featured" />

                                    <x-dashboard.form.input-checkbox resource-name="Destination" resource-desc="Is Country"
                                    error-key="country" name="country" id="country" label-title="Counrty" />

                            </div>
                            <div class="tab-pane fade" id="{{ 'featured-media-1' }}" role="tabpanel"
                                aria-labelledby="{{ 'featured-media-1' }}-tab">
                                <x-dashboard.form.media title="Add Banner Image" :images="old('banner')" name="banner" />

...
class ToursFetchCommand extends Command
{
    ...
    {
        $http = Http::baseUrl('https://mintravel.perfect-solution4u.com')
    ...
    private function saveTour(mixed $tour): void
    {
        ...
        foreach ($tour['destinations'] as $destination) {
            ...
            $dd = Destination::whereTranslation('slug', $destination['slug'])->firstOrNew();
            $dd->fill($dest_payload)->save();
            $destinations[] = $dd->id;
        }
        ...
        foreach ($tour['categories'] as $category) {
            ...
            $cc = Category::whereTranslation('slug', $category['slug'])->firstOrNew();
            $cc->fill($category_payload)->save();
            $categories[] = $cc->id;
        }
